...
class Agent(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        action = tf.cast(tf.random.categorical(logits=scores, num_samples=1), dtype=tf.int64)  # [B, 1]
        ...
        transp = tf.transpose(a=tf.stack([range_arr, action_idx]))
        chosen_relation = tf.gather_nd(next_relations, transp)

        #return loss, history, action probability distribution (logits), chosen action, chosen relation, initial neural network output
