...
class Episode(object):
    ...
    def __init__(self, graph, data, params, original_reward, rl_train=False):
        ...
        if original_reward:
            ...
            next_actions = self.grapher.return_next_actions(self.current_entities, self.start_entities, self.query_relation,
                                                        ...
                                                        self.num_rollouts, original_reward, answers = self.end_entities)
        else:
            if self.use_RL:
                # print("RL")
                next_actions = self.grapher.return_next_actions(self.current_entities, self.start_entities, self.query_relation,
                                                        self.all_answers, self.current_hop == self.path_len - 1,
                                                        self.num_rollouts, original_reward, masking = self.masked, rl = self.use_RL)
            else:
                # print("SL")
                next_actions = self.grapher.return_next_actions(self.current_entities, self.start_entities, self.query_relation,
                                                        self.all_answers, self.current_hop == self.path_len - 1,
                                                        self.num_rollouts, original_reward)
        self.state = {}
    ...
    def __call__(self, action):
        ...
        if self.original_reward:
            next_actions = self.grapher.return_next_actions(self.current_entities, self.start_entities, self.query_relation,
                                                        self.all_answers, self.current_hop == self.path_len - 1,
                                                        self.num_rollouts, self.original_reward, answers=self.end_entities)
        else:
            if self.use_RL:
                next_actions = self.grapher.return_next_actions(self.current_entities, self.start_entities, self.query_relation,
                                                        self.all_answers, self.current_hop == self.path_len - 1,
                                                        self.num_rollouts, self.original_reward, masking = self.masked, rl = self.use_RL)
            else:
                next_actions = self.grapher.return_next_actions(self.current_entities, self.start_entities, self.query_relation,
                                                        self.all_answers, self.current_hop == self.path_len - 1,
                                                        self.num_rollouts, self.original_reward)
...
class env(object):
    def __init__(self, params, reward_original, mode, rl):
        ...
        self.reward_original = reward_original
        self.RL = rl
        ...
        self.labeller = Labeller([self.grapher.array_store, params['entity_vocab']['PAD'], params['relation_vocab']['PAD'], self.correct_filepath, self.batcher.store_all_correct, self.reward_original, params['random_masking_coef']])
    ...
    def get_episodes(self):
        ...
        if self.mode == 'train':
            for data in self.batcher.yield_next_batch_train(self.labeller, self.RL):
                yield Episode(self.grapher, data, params, self.reward_original, rl_train = self.RL)
        else:
            for data in self.batcher.yield_next_batch_test(self.labeller):
                if data == None:
                    return
                yield Episode(self.grapher, data, params, self.reward_original)
