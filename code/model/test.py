...
# -*- coding: utf-8 -*-
"""
Created on Sun Jul  3 16:53:22 2022
...
class Trainer(object):
    def __init__(self, params, train_type, reward_type):
        ...
        # if rl, total iterations is already fine. Otherwise, we convert he
    ...
    def train(self, xdata, ydata_accuracy, ydata_loss):
        ...
        for episode in self.train_environment.get_episodes():
                ...
                for i in range(self.path_length):
                    ...
                    if self.rl:
                        ...
                        for batch_num in range(len(episode.correct_path[i])):
                          #  print("episode.correct_path[i][0]",i,episode.correct_path[i][0])
     
                          #  print('last_step[batch_num]',last_step[batch_num])
                            try:
           ...
           #                 if max(valid)>200:  
            #                    print(len(valid))
             #                   print('invalid',valid,last_step[batch_num])
              #                  print('np.array([batch_num]*len(valid), int),np.array(valid, int)',np.array([batch_num]*len(valid), int),np.array(valid, int))
